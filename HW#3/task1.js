...
function changeObject(obj, currency) {
    let copyObj = JSON.parse(JSON.stringify(obj)); //create a deep object copy;
    for (let i of copyObj['data']) {
        i['Currency'] = currency;
    }

    if (currency === "euro") {
        const coefficient = 0.92;
        for (let item of copyObj['data']) {
            item["Prices"]["Price"] = +(item["Prices"]["Price"] * coefficient).toFixed(2);
            item["Prices"]["Retail Price"] = +(item["Prices"]["Retail Price"] * coefficient).toFixed(2);
            item["Prices"]["Wholesale Price"] = +(item["Prices"]["Wholesale Price"] * coefficient).toFixed(2);
        }
    }
    console.log(copyObj);
    return copyObj;
...
let objEuro = changeObject(obj1, 'euro');
let objDollars = changeObject(obj1, 'dollars');